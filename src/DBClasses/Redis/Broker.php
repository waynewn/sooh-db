 ...
 * @author simon.wang
 */
class Broker extends Cmd implements \Sooh\DBClasses\Interfaces\DBReal
{
    ...
    public function getRecords($obj, $fields, $where=null, $sortgrpby=null,$pageSize=null,$rsFrom=0){
        ...
        * {@inheritDoc}
        * @see \Sooh\DBClasses\Interfaces\DB::updRecords()
        */
    ...
    public function addRecord($obj,$fields,$pkey=null){
		...
		if($chkkey){
			...
			if($exists){
				$err= new \Sooh\DBClasses\DBErr(\Sooh\DBClasses\DBErr::duplicateKey,'someelse are create same record',$this->lastCmd());
