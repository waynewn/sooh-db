<?php
namespace Sooh\DBClasses\Cases;
 ...
 *      前面判定是否有cache并获取cache
        ...
        $cached = \Sooh\DBClasses\Cases\CacheSimple::checkExists($arr4cache);
        if($cached){
            return $this->assignRes($cached);
        }
 * 
 *      后面生成数据后，保存cache
 *  \Sooh\DBClasses\Cases\CacheSimple::saveCache($arr4cache, $list);
 * 
 * @author wangning
 */
class CacheSimple extends \Sooh\DBClasses\KVObj{
