 ...
 * @author simon.wang
 */
class AccountLog extends \Sooh\DBClasses\KVObj{
    /**
     * 余额最小值，任何订单都会检查
     * @var int
     */
    ...
    /**
     *
     * @var \Sooh\DBClasses\Interfaces\DB 
     */
    protected $__db;
    protected $__tb;
    /**
    ...
    public function getHistory($where=array(),$pagesize=30)
    {
        if(is_numeric($pagesize)){
            $pager = new \Sooh\DBClasses\Pager($pagesize);
            $pager->init(-1,1);
        }else{
            $pager = $pagesize;
        }
     ...
     * @param type $userId
     * @return \Sooh\DBClasses\Cases\AccountLog
     */
